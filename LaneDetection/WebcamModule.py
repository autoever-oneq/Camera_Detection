...
class webcamModule:
  def __init__(self, size: tuple=dsize):
    self.cap = cv2.VideoCapture(f'nvarguscamerasrc ! video/x-raw(memory:NVMM), width=3264, height=1848, format=(string)NV12, framerate=(fraction)28/1 !
...
if __name__ == '__main__':
  ...
  try:
    while webcam.cap.isOpened():
      ...
      curveVal = laneDetection.getLaneCurve(img, laneDiff=0)
      print(curveVal)
